...
from modules.models import DFC_LeNet_5, DFC_RetiNet
...
retinet = DFC_RetiNet(retinic_kernel_size = rks,
                      in_channels = in_channels).eval()

reti_state_dict = torch.load(
    f'{PATH_TO_STATEDICTS}/trained_DFC_RetiNet_{rks}_state_dict.pt',
    map_location = device)
...
axs[0,1].set_title(f"Retinic hidden output (ks = {rks})", fontsize=title_size, pad=title_pad)


# Save and show
PATH_TO_SAVE = os.path.join(
    PATH_TO_SAVE, f"boxex_retinet_{rks}_{dataset_name}"
)
