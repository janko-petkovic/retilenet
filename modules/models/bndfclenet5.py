# Batch normed fully connected version of LeNet 5: the c3 layer does not follow
...
def test(in_ch):
  ...
  device = 'cuda' if torch.cuda.is_available() else 'cpu'
  model = BNDFC_LeNet_5(in_ch).to(device)
  input = torch.zeros(1,in_ch,32,32).to(device)
